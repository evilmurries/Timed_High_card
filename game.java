...
class Model {
   ...
   public int getNumberOfCardsInPlayArea() {
   ...
   public Card getCardInPlayArea(int index) {
      if (index < 0 || index >= playArea.length) {
         return new Card('Z', Card.Suit.SPADES);
      }
      return this.playArea[index];
   }
  ...
  /* public void comp() {
      ...
      //changed cardToplay
      if (computerCard.compareTo(computerCard) < 0) {
   ...
   public void afterAllMoved() {
      ...
      if (Controller.compWinner) {
        // Controller.lastPlayedCard = makeComputerMove();
      }
   }
   ...
   public void computerMove() {
      ...
      while (hasCard) {
         Timer timer = new Timer(7000, gameController.getNewDelayedListener());
         timer.setRepeats(false);
         timer.start();
         for (int i = 0; i < gameController.getGame().getHand(0).getNumCards(); i++) {
            ...
            int playAreaCardIndex = getCardInPlayAreaThatSmallerOrBigger(cardToInspect);
            if (playAreaCardIndex == 0 || playAreaCardIndex == 1) {

               Card cardToPlay = gameController.getGame().getHand(0).playCard(i);
               putCardInPlayArea(playAreaCardIndex, cardToPlay);
               gameController.requestSetPlayedCard(playAreaCardIndex, cardToPlay);
               
               Timer timer1 = new Timer(7000, this.gameController.getRefreshCompCardListener(cardToPlay));
               timer1.setRepeats(false);
               timer1.start();
               System.out.println("Computer is playing: " + cardToPlay);
         ...
         if (gameController.getGame().getNumCardsRemainingInDeck() > 0
               && gameController.getGame().getHand(0).getNumCards() < 7) {
            Card cardToTake = gameController.getGame().getCardFromDeck();
   ...
   public void makeComputerMove() {
      int computerCardIndex = chooseCompHighestCard();

      Card computerCard = gameController.getGame().getHand(0).playCard(computerCardIndex);
    ...
    */
   public int chooseCompHighestCard() {
...
class CardTableView extends JFrame {
   ...
   /*
    * Hides current window and notifies controller that the game has been stopped.
    */
   public void stopGame() {
   ...
   public void notifyPlayerMadeMove(Card cardToPlay) {
      this.playedCardLabels[1].setIcon(GUICard.getIcon(cardToPlay));
   }
...
class Controller {
   ...
   /*
    * Notifies model that the game has been stopped.
    */
   public void stopGame() {
      gameModel.stopGame();
   }
   ...
   public StartStopListener getStartStopController() {
      return new StartStopListener(this, this.gameModel);
   }
